...
//                   System.out.println(date);
                   ...
                   try {
                       String lg = txtID.getText().trim();
                        PreparedStatement pr = conn.prepareCall("{CALL SP_ADDLOGIN(?,?,?)}"); // tao login
                        pr.setString(1, lg);
                        pr.setString(2, "123");
                        pr.setString(3, "QLDSV_TC");
                        pr.execute();
                        pr = conn.prepareCall("{CALL SP_GRANTDBACCESS( ?, ?)}"); // mapping user
                        pr.setString(1, lg);
                        pr.setString(2, lg);
                        pr.execute();
                        ...
                        // them login vao role 
                        pr = conn.prepareCall("{CALL sp_addrolemember(?,?)}");
                        pr.setString(1, "SV");
                        pr.setString(2, lg);
                        pr.execute();
        ...
        try {
            ...
            if(rs.next()!=false){
                ...
                if(rs1.next()!=false){
                    ...
                    try {
                        PreparedStatement pr = Main.kn.prepareStatement("{Call sp_droplogin (?)}");
                        pr.setString(1,txtID.getText().trim()); // masv
                        pr.execute();
                        pr = Main.kn.prepareStatement("{Call sp_dropuser(?)}");
                        pr.setString(1, txtID.getText().trim()); // masv
                        pr.execute();
                        System.out.println("xoa login, user thanh cong");
                    }
